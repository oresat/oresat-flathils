...
```sh
flathils example-harness
```

You can run Hardware-in-Loop tests with the `--run-hil` flag:

```sh
flathils example-harness --run-hil
```
...
```sh
flathils can-harness --run-hil --can-device can0 -v
```
...
```sh
flathils bootloader-harness --run-hil --can-device can0 --image-path ~/path/to/zephyr.signed.bin
```
