...
DEFAULT_BIN_PATH = Path(
    "/home/derek/psas/oresat-flathils/src/oresat_flathils/harnesses/mcuboot_harness/zephyr_image/zephyr.signed.bin"
)
...
def parse_args():
    ...
    p.add_argument(
        "--confirm", dest="confirm_image", action="store_true", default=DEFAULT_CONFIRM_IMAGE
    )
...
def flash_device(
    ...
    block_transfer=DEFAULT_BLOCK_TRANSFER,
    download_buffer_size=DEFAULT_DOWNLOAD_BUFFER_SIZE,
    status_timeout=DEFAULT_STATUS_TIMEOUT_S,
    bootup_timeout=DEFAULT_BOOTUP_TIMEOUT_S,
    sdo_timeout=DEFAULT_SDO_TIMEOUT_S,
    sdo_retries=DEFAULT_SDO_RETRIES,
    confirm_image=DEFAULT_CONFIRM_IMAGE,
    request_crc=DEFAULT_REQUEST_CRC,
):
    """Flash a built zephyr image with CANopen.

    Either retunrs a 0 on success or a failure #.
    mainly based off flash_canopen.py on template app
    with slight modifications to allow to return with pytest.
    """
    bin_path = Path(bin_path)

    if not bin_path.is_file():
        logging.error(f"No binary file found on: {bin_path}")
        return 1

    size = os.path.getsize(bin_path)

    network = canopen.Network()
    network.connect(interface="socketcan", channel=channel, bitrate=bitrate)
    ...
    try:
        ...
        status = wait_flash_status_ok(flash_sdo, status_timeout)
        if status != 0:
        ...
        node.nmt.wait_for_bootup(timeout=bootup_timeout)
...
def main():
    ...
    result = flash_device(
        ...
        bootup_timeout=args.bootup_timeout,
        sdo_timeout=args.sdo_timeout,
        sdo_retries=args.sdo_retries,
        confirm_image=args.confirm_image,
        request_crc=args.request_crc,
    )

    end_time = time.perf_counter()
    total_flash_time = end_time - start_time
    print(f"Total time to flash: {total_flash_time:.4f} seconds")
    return result
